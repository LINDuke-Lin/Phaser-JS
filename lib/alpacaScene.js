...
class alpacaScene extends Phaser.Scene {
  ...
  #playSound(_self) {
    let isPlay = _self.skillMusic.isPlaying;
    if (isPlay === false) _self.skillMusic.play();
  }
  ...
  preload() {
    ...
    this.load.audio("music1", "./assets/music/background.mp3");
    this.load.audio("skillMusic", "./assets/music/file.mp3");
  ...
  create() {
    ...
    this.skillMusic = this.sound.add("skillMusic", {
      volume: 0.8, //音量
      loop: false, // 是否輪播
    });

    this.skillMusic.addMarker("fileTime", 1);
